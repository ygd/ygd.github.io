...
for date in dates
    ...
    write_template_file(yearpage_path, "archive/#{date[:year]}/", date[:year], {year:"#{date[:year]}"})

    monthpage_path = dates_folder_path + "/#{date[:year]}-#{date[:month]}.md"
    month_name = "#{MONTH_NAMES[Integer(date[:month])]} #{date[:year]}"
    write_template_file(monthpage_path, "archive/#{date[:year]}/#{date[:month]}/", month_name, {year: date[:year], month: date[:month]})
end
